...
dsh 的网页用户面前没有终端，所以引擎设置有一张卡片，在**设置 → 插件 → 插件配置**里（dsh 0.1.7 起改到侧栏的**插件**页，在**已安装**里点开 `@liustack/modlens`）：用哪个引擎读图、它的密钥、地址、模型和代理方式，以及一次读取可以借用本机哪些已有登录。展开卡片（0.1.7 起是打开这个页面）时会探测本机，只列出真正找到的 harness，让授权是在真实选项之间做选择，而不是面对五个名字。
...
```sh
npx -y @deepseek-ai/dsh plugin --profile <name> add @liustack/modlens@3.26.5
```
